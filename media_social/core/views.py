    ...
    if request.method == 'POST':
        ...
        if password == password1:
            if User.objects.filter(email=email).exists():
                messages.info(request, 'Email déja utiliser')
                return redirect('creationCompte')
            elif User.objects.filter(username=username).exists():
                messages.info(request, 'pseudo déja utiliser')
                return redirect('creationCompte')
            else:
            ...
            messages.info(request,'Le mot de passe ne correspond pas')
            return redirect('creationCompte')
    else:
...
def setting(request):
    ...
    if request.method == 'POST':
        ...
        if request.FILES.get('image') == None:
            ...
            couverture = request.POST['image1']
